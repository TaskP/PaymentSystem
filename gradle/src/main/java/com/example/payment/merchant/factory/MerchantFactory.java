...
public interface MerchantFactory {
    ...
    ValidationException getValidationException(Merchant merchant);

    default boolean isValid(final Merchant merchant) {
        return getValidationException(merchant) == null;
    }

    default void validate(final Merchant merchant) throws ValidationException {
        final ValidationException valEx = getValidationException(merchant);
        if (valEx != null) {
            throw valEx;
        }
    }
